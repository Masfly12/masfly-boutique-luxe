...
import {
  CheckCircle, XCircle, Store, Mail,
...
export function VendorManager() {
  ...
  const approveMutation = useMutation({
    mutationFn: async (vendor: any) => {
      const { error } = await (supabaseAuth.rpc as any)("admin_approve_vendor_payment", {
  ...
  const rejectMutation = useMutation({
    mutationFn: async (vendor: any) => {
      const { error } = await (supabaseAuth.rpc as any)("reject_vendor", {
        _vendor_id: vendor.id,
  ...
  const filtered = vendors?.filter((v: any) =>
    filter === "all" ? true : v.approval_status === filter
  );
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 border border-border" style={{ background: "var(--card)" }}>
          ...
          <p className="text-xs font-body text-muted-foreground tracking-wide">Aucun vendeur</p>
        </div>
      ) : (
              ...
              >
                ...
                {vendor.approval_status !== "approved" && (
                  <div className="flex gap-2 mt-3 pt-3 border-t border-border">
                    ...
                    <button
                      onClick={() => setSelectedVendor({ ...vendor, action: "reject" })}
                    ...
                    {vendor.whatsapp && (
                      <a
                        ...
                        className="flex items-center justify-center px-3 border border-border text-muted-foreground hover:text-foreground transition-colors"
                      >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <p className="text-[9px] font-body tracking-[0.3em] uppercase" style={{ color: GOLD }}>
                Approuver — {selectedVendor.shop_name || selectedVendor.name}
              </p>
            </div>
            <div className="space-y-2">
              <label className="text-[9px] font-body tracking-[0.25em] uppercase text-muted-foreground">Montant reçu (FCFA)</label>
              <input
                type="number"
                value={paymentAmount}
            ...
            <div className="space-y-2">
              <label className="text-[9px] font-body tracking-[0.25em] uppercase text-muted-foreground">Référence de paiement</label>
              <input
                type="text"
                value={paymentRef}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => setSelectedVendor(null)}
                className="px-5 font-body text-[10px] tracking-[0.2em] uppercase border border-border text-muted-foreground hover:text-foreground"
              >
                Annuler
              </button>
            </div>
        ...
        >
          ...
          >
            <p className="text-[9px] font-body tracking-[0.3em] uppercase text-destructive">
              Rejeter — {selectedVendor.shop_name || selectedVendor.name}
            </p>
            <div className="space-y-2">
              <label className="text-[9px] font-body tracking-[0.25em] uppercase text-muted-foreground">Motif (optionnel)</label>
              <textarea
                value={rejectNote}
                onChange={(e) => setRejectNote(e.target.value)}
                rows={3}
                className="w-full px-3 py-2 text-sm border border-border bg-background resize-none focus:outline-none"
              />
            </div>
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 font-body text-[10px] tracking-[0.2em] uppercase py-3 bg-destructive text-white hover:opacity-90 disabled:opacity-50"
              >
              ...
              <button
                onClick={() => setSelectedVendor(null)}
                className="px-5 font-body text-[10px] tracking-[0.2em] uppercase border border-border text-muted-foreground"
              >
                Annuler
              </button>
            </div>
